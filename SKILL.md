...
| 格式 | 转换能力 | 当前限制 |
|------|----------|----------|
| `.md` | 标题、加粗/斜体、列表（含多级嵌套）、表格、引用、代码块、链接 | 无已知限制 |
| `.docx` | 段落、标题样式、加粗/斜体、表格、**内嵌图片自动提取并按文档顺序嵌入正文** | 无已知限制 |
| `.xlsx` | 全部工作表 → 表格（表头加粗），**内嵌图片自动提取并附于表格之后** | XML 原生图表不渲染；内嵌图片统一附于文末（XLSX 不支持图片与单元格精确绑定） |
| `.csv` | 整表 → 表格（首行表头加粗），**已用横向滚动容器包裹** | 无已知限制 |
