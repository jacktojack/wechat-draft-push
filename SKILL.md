...
| `.xlsx` | 全部工作表 → 表格（表头加粗），**内嵌图片自动提取并附于表格之后** | 显示单元格内存储的计算结果（即正确值，不显示公式文本）；XML 原生图表不渲染，但以图片形式插入的图表会随内嵌图片自动提取；图片统一附于文末（XLSX 不支持图片与单元格精确绑定） |
| `.csv` | 整表 → 表格（首行表头加粗），**已用横向滚动容器包裹** | 宽表在卡片内横向滚动，不再撑破手机端页面布局 |
